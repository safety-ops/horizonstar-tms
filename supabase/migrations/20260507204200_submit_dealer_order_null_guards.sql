-- Migration: 20260507204200_submit_dealer_order_null_guards.sql
-- Purpose : Harden submit_dealer_order() with explicit NOT NULL guards (task #18 / C2 nit).
--           Replaces the raw Postgres not-null constraint error (SQLSTATE 23502) with a
--           clean RAISE EXCEPTION using ERRCODE 22023 (invalid_parameter_value) so callers
--           receive an actionable message instead of an internal constraint violation string.
--
-- Parent  : 20260507204100_submit_dealer_order_rpc.sql
--           Function body is copied verbatim from that migration; only the guard block is
--           inserted (after dealer_code resolution, before next_order_number call).
--
-- The COMMENT ON FUNCTION is updated to reflect the 7 required fields.
...
BEGIN
    ...
    -- 4. Required-field guard (task #18).
    --    Fires BEFORE next_order_number() to avoid burning a sequence slot on bad input.
    --    Uses NULLIF(...,'') so empty-string payloads are treated the same as NULL.
    --    ERRCODE 22023 = invalid_parameter_value (not 23502 / not-null constraint).
    IF NULLIF(p_payload->>'pickup_location', '')    IS NULL
    OR NULLIF(p_payload->>'delivery_location', '')  IS NULL
    OR NULLIF(p_payload->>'payment_type', '')        IS NULL
    OR NULLIF(p_payload->>'payment_amount', '')      IS NULL
    OR NULLIF(p_payload->>'vehicle_make', '')        IS NULL
    OR NULLIF(p_payload->>'vehicle_model', '')       IS NULL
    OR NULLIF(p_payload->>'vehicle_year', '')        IS NULL
    THEN
        RAISE EXCEPTION 'Missing required fields: pickup_location, delivery_location, payment_type, payment_amount, vehicle_make, vehicle_model, vehicle_year are all required'
            USING ERRCODE = '22023';
    END IF;

    -- 5. Mint order number via the existing locked RPC.
    ...
    -- 7. Audit row via the existing SECURITY DEFINER RPC.
...
-- Re-state grants explicitly for documentation clarity.
-- CREATE OR REPLACE preserves existing grants but explicit re-statement is canonical here.
...
Required payload keys (ERRCODE 22023 raised if any are missing or empty-string):
  pickup_location, delivery_location, payment_type, payment_amount,
  vehicle_make, vehicle_model, vehicle_year.

Optional payload keys:
  vehicle_vin, vehicle_color, vehicle_body_type, vehicle_lot_number,
  vehicle_buyer_number,
  pickup_address, pickup_city, pickup_state, pickup_zip,
  pickup_contact_name, pickup_contact_phone,
  delivery_address, delivery_city, delivery_state, delivery_zip,
  delivery_contact_name, delivery_contact_phone,
  pickup_date, delivery_date, payment_terms, notes,
