-- Rollback: 20260507203100_log_activity_details_size_cap.sql
-- Restores public.log_activity to the pre-cap version
-- (functionally identical to 20260507202900 + 20260507203000).
...
BEGIN
    ...
    IF v_uid IS NULL THEN
        RAISE EXCEPTION 'Unauthenticated audit attempt' USING ERRCODE = '42501';
    END IF;

    SELECT u.id::BIGINT, u.email::TEXT,
           NULLIF(trim(coalesce(u.first_name, '') || ' ' || coalesce(u.last_name, '')), '')
    INTO   v_user_id, v_user_email, v_user_name
    FROM   public.users u WHERE u.auth_id = v_uid LIMIT 1;

    IF v_user_id IS NULL THEN
        SELECT au.email::TEXT INTO v_user_email
        FROM   auth.users au WHERE au.id = v_uid LIMIT 1;
        IF v_user_email IS NULL THEN v_user_email := 'auth:' || v_uid::TEXT; END IF;
        v_user_name := NULL;
    END IF;
    ...
    IF v_ip IS NULL OR v_ip = '' THEN v_ip := 'rpc'; END IF;
    IF char_length(v_ip) > 50 THEN v_ip := left(v_ip, 50); END IF;

    INSERT INTO public.activity_log (
        user_id, user_email, user_name, action, details, ip_address, created_at
    ) VALUES (
        v_user_id, v_user_email, v_user_name, p_action, p_details, v_ip, NOW()
